...
If a custom `shouldComponentUpdate` is provided, it is consulted when the props change (because the parent passes new props) or the state changes (as a result of calling `setState`),
